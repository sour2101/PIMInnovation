    ...
    .run([
      ...
      function ($templateCache) {
        $templateCache.put('template/treeGrid/treeGrid.html',
          ...
          "       <th ng-repeat=\"col in colDefinitions\"><a style=\"color: #888;\" ng-if=\"col.sortable\" ng-click=\"sortBy(col)\">{{col.displayName || col.field}}</a><span ng-if=\"!col.sortable\">{{col.displayName || col.field}}</span><i ng-if=\"col.sorted\" class=\"{{col.sortingIcon}} pull-right\"></i></th>\n" +
          "        <th>Action</th>\n " +
          "     </tr>\n" +
          "   </thead>\n" +
          "   <tbody>\n" +
          ...
          "       <td width=\"10%\"><a ng-click=\"user_clicks_branch(row.branch)\"><i ng-class=\"row.tree_icon\"\n" +
          ...
          "       </td>\n" +
          "       <td ng-hide=\"row.level ===1 && roleUser !== 0\" style=\"width:100%;float:left;\"><div class=\"form-inline\" style=\"width:100%;float:left;\"><div  class=\"select-btn\" style=\"float:left;\"> <a href=\"\" ng-hide=\"row.level ===2 && roleUser !== 0\" ng-click=\"Edit(row.branch)\"><span class=\"glyphicon glyphicon-pencil\"></span></a>&nbsp;<a href=\"\" ng-click=\"Add(row.branch)\" ng-hide=\"row.branch.level==5\"  ><span class=\"glyphicon glyphicon-plus\"></span></a></div><div ng-hide=\"row.level ===2 && roleUser !== 0\" style=\"float:left;\"><input id=\"cmn-toggle-{{row.branch.structureName}}{{row.branch.id}}\"  ng-model=\"row.branch.active\" ng-change=\"Delete(row.branch)\" class=\"cmn-toggle cmn-toggle-round\" type=\"checkbox\"><label for=\"cmn-toggle-{{row.branch.structureName}}{{row.branch.id}}\"></label></div></div></td>\n" +
          "     </tr>\n" +
          "   </tbody>\n" +
          " </table>\n" +
          "</div>\n" +
        ...
        return {
          ...
          link       : function (scope, element, attrs) {
            ...
            scope.Delete = function (branch) {
                scope.$parent.DeleteOrg(branch);
            };
            scope.Add = function (branch) {
                scope.$parent.AddOrg(branch);
            };
            scope.Edit = function (branch) {
                scope.$parent.EditOrg(branch);
            };
